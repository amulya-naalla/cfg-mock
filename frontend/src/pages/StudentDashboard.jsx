...
function ContinueCard({ row, onStart, lang }) {
  const c = row.content;
  return (
    <div className="student-continue-card">
      <div className="student-continue-main">
        ...
        <div className="content-meta">
          <span className="meta-tag">{t(c.subject.toLowerCase(), lang) || c.subject}</span>
...
function RecommendationCard({ rec, onStart, lang }) {
  const c = rec.content;
  return (
    <article className="content-card student-rec-card">
      ...
      <div className="content-actions">
        <button className="btn-primary" onClick={() => onStart(c)}>{t('start_learning', lang)}</button>
      </div>
    </article>
  );
}
    ...
    <div className="page-dashboard student-page">
      ...
      <div className="student-welcome">
        <div className="student-welcome-text">
          <h1 className="page-title">{t('welcome_back', lang)}, {student.name.split(' ')[0]}! 👋</h1>
          <p className="page-subtitle">
            Grade {student.grade} • Age {student.age} • {t('welcome_sub', lang)}
          </p>
        </div>
        <button className="quick-btn quick-btn-primary" onClick={() => navigate('/student/learn')}>
          🚀 {t('start_learning', lang)}
        </button>
      </div>
      ...
      <div className="metrics-row student-metrics-row">
        ...
        <div className="metric-card student-metric-card">
          ...
          <span className="metric-label">{t('level_status', lang)}</span>
      ...
      {continueItem ? (
        <section className="panel student-continue-panel">
          <div className="panel-header">
            <h2>▶ {t('continue_learning', lang)}</h2>
            <span className="benchmark-note">pick up where you left off</span>
          </div>
          <ContinueCard row={continueItem} onStart={openContent} lang={lang} />
        </section>
      ) : (
        <section className="panel student-continue-panel">
          <div className="panel-header">
            <h2>▶ {t('continue_learning', lang)}</h2>
            <span className="benchmark-note">{t('recommendation_note', lang)}</span>
          </div>
      ...
      <div className="dashboard-grid student-dashboard-grid">
        ...
        <div className="dashboard-col">
          ...
          <section className="panel panel-recommended">
            <div className="panel-header">
              <h2>🎯 {t('recommended_title', lang)}</h2>
              <span className="benchmark-note">{t('recommendation_note', lang)}</span>
            </div>
            {recommended.length === 0 ? (
              ...
              <div className="content-grid student-rec-grid">
                {recommended.map((rec) => (
                  <RecommendationCard key={rec.content._id} rec={rec} onStart={openContent} lang={lang} />
                ))}
              </div>
            )}
          </section>
